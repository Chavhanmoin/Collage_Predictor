...
def predict_colleges(sample_input, top_n=5):
    try:
        transformed_input = {
    ...
    top_colleges = [label_to_college.get(idx, f"Unknown College ID: {idx}") for idx in top_indices]
...
def predict():
    if 'username' in session:
        try:
            ...
            top_colleges = predict_colleges(sample_input)

            if isinstance(top_colleges, str):  # error message returned
                return top_colleges, 400
